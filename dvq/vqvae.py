...
class VQVAE(pl.LightningModule):
    def __init__(self, args, input_channels=3):
        ...
        self.decoder = Decoder(
            n_init=args.embedding_dim, n_hid=args.n_hid, output_channels=input_channels
        )
        ...
        self.quantizer = QuantizerModule(
            self.encoder.output_channels, args.num_embeddings, args.embedding_dim
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        encodings = (
            F.one_hot(ind, self.quantizer.n_embed)
            .float()
            .reshape(-1, self.quantizer.n_embed)
        )
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(
            optim_groups, lr=3e-4, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-4
        )
    ...
    def add_model_specific_args(parent_parser):
        ...
        # model type
        parser.add_argument(
            "--vq_flavor", type=str, default="vqvae", choices=["vqvae", "gumbel"]
        )
        parser.add_argument(
            "--enc_dec_flavor",
            type=str,
            default="deepmind",
            choices=["deepmind", "openai"],
        )
        parser.add_argument(
            "--loss_flavor", type=str, default="l2", choices=["l2", "logit_laplace"]
        )
        # model size
        parser.add_argument(
            "--num_embeddings",
            type=int,
            default=512,
            help="vocabulary size; number of possible discrete states",
        )
        parser.add_argument(
            "--embedding_dim",
            type=int,
        ...
        parser.add_argument(
            ...
            type=int,
            default=64,
            help="number of channels controlling the size of the model",
        )
        return parser
...
def cli_main():
    ...
    # training related - manual trainer arguments since add_argparse_args is deprecated
    ...
    # Create trainer with manual arguments instead of from_argparse_args
    trainer = pl.Trainer(
